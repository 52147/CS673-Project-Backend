...
def get_plate_result(img,session_rec): #识别后处理
    img =rec_pre_precessing(img) # (1, 3, 48, 168)
    y_onnx = session_rec.run([session_rec.get_outputs()[0].name], {session_rec.get_inputs()[0].name: img})[0] # (1, 21, 78)
...
if __name__ == "__main__":
    onnx_file="/media/dm/新加卷/SDK/tnn-lib/TNN_build/tools/onnx2tnn/onnx-converter/workspace/data/pretrained/sim/crnn_sim.onnx"
    parser = argparse.ArgumentParser()
    parser.add_argument('--onnx_file', type=str, default=onnx_file, help='model.pt path(s)')#识别模型
